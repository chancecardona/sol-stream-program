...
impl Processor {
    pub fn process(
        ...
        match instruction {
            ...
            StreamInstruction::CloseStream => {
                Self::process_close_stream(program_id, accounts)
            },
        }
    }
    ...
    fn process_create_stream(
        ...
        //.key returns an: &'a Pubkey (need to dereference to compare)
        if *admin_account.key != admin_pub_key {
            return Err(StreamError::AdminAccountInvalid.into())
        }

        // 0.0003 sol token admin account fee
        // 30000000 Lamports = 0.03 sol
        // try_borrow_mut_lamports returns a "Ref" to an "&mut u64" if not an error (so unwrap,
        // deref the Ref to get &mut u64, then deref again to get u64.
        **escrow_account.try_borrow_mut_lamports()? -= 300000;
        **admin_account.try_borrow_mut_lamports()? += 300000;
    ...
    fn process_withdraw(
        ...
        // Use the amount/second * the seconds elapsed to determine how many left
        ...
        **receiver_account.try_borrow_mut_lamports()? += data.amount;
        ...
        // borrow_mut returns a RefMut<'_, T>, so dereference to get T which is &mut [u8].
        // Then need a mutable reference to that because serialize wants it to be a mutable 
        // reference to something that implements Write, 
        // so we give a mut ref to the mut ref so we can keep track where in the array we are
        escrow_data.serialize(&mut *escrow_account.data.borrow_mut())?;
        Ok(())
    }
